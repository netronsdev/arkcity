...
아크시티 게임의 뽑기(가챠), 합성, 거래 기록, 경험치↔에테르 교환 배율, 보급로봇 박스가 공정하게
운영되고 있는지 **누구나 독립적으로 검증**할 수 있는 오픈소스 코드입니다.
...
| **박스 확률** | 보급로봇의 전투강화 스크롤 상자 확률표(`loot_box.py`)와 서버 응답 비교 |
| **박스 결과** | seed_hash로 투명성 페이지에서 박스 오픈 결과를 영구 재조회 |
...
loot_box.py              ← 서버에서 실제 사용하는 박스 확률 엔진 (동일 사본)
...
보급로봇에서 판매되는 **전투강화 스크롤 상자**(개당 100,000 에테르)는 1개 오픈 시 2~10,000장의
스크롤이 무작위로 지급됩니다. 추첨은 가챠와 동일한 Commit-Reveal 방식으로 공정하게 결정됩니다.

### 박스 확률표 (rate_table_version: `box_combat_scroll_v1`)

| 등급 | 보상 | 가중치 | 확률 |
|------|------|--------|------|
| 잭팟 | **10,000개** | 1,135 | **0.01135%** |
| 초대박 | 1,000개 | 11,350 | 0.1135% |
| 대박 | 100개 | 113,500 | 1.135% |
| 중상 | 50개 | 227,000 | 2.270% |
| 중 | 20개 | 567,500 | 5.675% |
| 소상 | 10개 | 1,135,000 | 11.35% |
| 평범 | 5개 | 5,003,657 | 50.04% |
| 꽝 | 2개 | 2,940,858 | 29.41% |
| **합계** | | **10,000,000** | **100%** |

**기대값(EV) = 9.90개 / 박스** — 박스 1개당 평균 9.9개의 스크롤이 나옵니다.
박스 단가 100,000 에테르 ÷ 9.9 ≈ **개당 약 10,101 에테르**.

### 박스 결과 검증

박스 오픈 직후 결과창에 표시되는 `seed_hash`(64자리 16진수)를 사용해 결과를 영구 재조회할 수 있습니다.

**웹에서 확인:**
```
https://netrons.co.kr/game/transparency/box?hash=<seed_hash>
```
- 박스 수, 총 획득 스크롤, 등급별 카운트가 영구히 동일하게 표시됨
- 등급별 카운트 합 = 박스 수 (자동 검증)

**확률표 일치성 확인:**
```bash
python -c "from loot_box import public_rate_table; import json; print(json.dumps(public_rate_table(), indent=2, ensure_ascii=False))"
```
이 출력과 서버의 `GET /game/api/box/rates` 응답이 동일하면 서버가 이 코드와 같은 확률표를 사용 중임이 증명됩니다.

### 박스 추첨 로직 (loot_box.py)

박스 N개 일괄 오픈 시:
1. 서버가 32바이트 무작위 시드 생성 → SHA-256 해시 저장 (`seed_hash`)
2. 박스마다 derived nonce (`base_nonce#0`, `base_nonce#1`, ...)로 `reveal_randint(seed, nonce, 0, 9_999_999)` 계산
3. 누적 가중치 밴드(`_CUMULATIVE_BANDS`)에 매핑하여 등급 결정
4. N개 결과를 합산하여 1개 DB 행에 저장 (`box_open_sessions` 테이블)

같은 (seed, nonce) 조합은 영원히 같은 결과를 재현합니다.
